...
// スタッフ選択ステップ
const StaffSelectionStep = ({ date, slots, selectedSlot, setSelectedSlot, formData, error, onNext, onClose }) => (
  <>
    <h2>{date} のスタッフ選択</h2>
        ...
        <div key={slot.id} style={{ marginBottom: 8, borderBottom: '1px solid #eee', paddingBottom: 4 }}>
          <label>
            <input
              ...
              name="slot"
              value={slot.id}
              disabled={slot.status !== 'available' || slot.remaining < formData.people}
    ...
    <div style={{ marginTop: 16, display: 'flex', gap: 8 }}>
      ...
      <button onClick={onNext} style={{ flex: 1, background: '#28a745', color: '#fff' }}>次へ</button>
    </div>
  </>
);

// 情報入力ステップ
const InfoInputStep = ({ formData, setFormData, error, onNext, onBack }) => (
  <>
    <h2>ご予約情報入力</h2>
    <div style={{ display: 'grid', gap: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button onClick={onBack} style={{ flex: 1 }}>戻る</button>
        <button 
          onClick={onNext} 
          style={{ 
            flex: 1, 
...
// 確認ステップ
const ConfirmationStep = ({ date, selectedSlot, formData, error, loading, onNext, onBack }) => (
  <>
    <h2>ご予約内容の確認</h2>
    <div style={{ 
      background: '#f8f9fa', 
...
// 完了ステップ
const CompletionStep = ({ date, selectedSlot, formData, onClose }) => (
  <>
    <h2>ご予約完了</h2>
...
// メインのBookingDrawerコンポーネント
export default function BookingDrawer({ open, date, slots, onClose, onSubmit }) {
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      setError('予約処理に失敗しました。もう一度お試しください。');
      setStep(2);
    } finally {
      setLoading(false);
    }
